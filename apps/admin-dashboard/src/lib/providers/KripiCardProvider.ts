...
export class KripiCardProvider implements CardProvider, FinancialProvider {
  ...
  async approveTransaction(transactionId: string): Promise<boolean> {
    ...
    try {
      ...
      ProviderLogger.error(this.name, 'approveTransaction', `Provider approval endpoint failed: ${e.message}`);
      throw e;
    }
  }
  ...
  async rejectTransaction(transactionId: string): Promise<boolean> {
    ...
    try {
      ...
      ProviderLogger.error(this.name, 'rejectTransaction', `Provider rejection endpoint failed: ${e.message}`);
      throw e;
    }
  }
}
