...
const COLLATERAL_ASSETS = [
  { symbol: 'BTC', name: 'Bitcoin', ltv: '70%', color: '#F7931A' },
  { symbol: 'ETH', name: 'Ethereum', ltv: '65%', color: '#627EEA' },
  { symbol: 'SOL', name: 'Solana', ltv: '50%', color: '#14F195' },
];
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={[styles.headerTitle, { color: T.text }]}>Credit</Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoGrid}>
          <View style={[styles.infoCard, { backgroundColor: T.surface, borderColor: T.border }]}>
            <Feather name="zap" size={20} color={T.primary} style={{ marginBottom: 8 }} />
            <Text style={[styles.infoTitle, { color: T.text }]}>Instant</Text>
            <Text style={[styles.infoDesc, { color: T.textDim }]}>Funds available immediately.</Text>
          </View>
          <View style={[styles.infoCard, { backgroundColor: T.surface, borderColor: T.border }]}>
            <Feather name="shield" size={20} color={T.success} style={{ marginBottom: 8 }} />
            <Text style={[styles.infoTitle, { color: T.text }]}>Secure</Text>
            <Text style={[styles.infoDesc, { color: T.textDim }]}>Fully collateralized on-chain.</Text>
          </View>
        </View>

        {/* Supported Collateral List */}
        <Text style={[styles.sectionTitle, { color: T.text }]}>Accepted Collateral</Text>

        <View style={[styles.listContainer, { backgroundColor: T.surface, borderColor: T.border }]}>
          {COLLATERAL_ASSETS.map((asset, index) => (
            <View key={asset.symbol}>
              <View style={styles.assetRow}>
                ...
                <View style={styles.assetRight}>
                  <Text style={[styles.assetApy, { color: T.text }]}>{asset.ltv}</Text>
                  <Text style={[styles.assetApyLabel, { color: T.textDim }]}>Max LTV</Text>
                </View>
              </View>
              {index < COLLATERAL_ASSETS.length - 1 && (
