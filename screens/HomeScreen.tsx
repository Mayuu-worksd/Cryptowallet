        ...
        style={[
          {
            ...
            height,
            borderRadius,
            backgroundColor: T.border ?? "#2a2a2a",
            opacity: anim,
          },
          style,
        ]}
...
const CoinIcon = memo(
  ({ symbol, size = 44 }: { symbol: string; size?: number }) => {
    ...
    return (
      ...
      >
        <Text
          style={{ color, fontSize: size * 0.38, fontFamily: Fonts.extraBold }}
        >
          {symbol.charAt(0)}
        </Text>
      </View>
          ...
          style={[
            {
              ...
              height: 60,
              borderRadius: 30,
              backgroundColor: isDark ? "#1c1b1b" : "#F1F3F4",
              alignItems: "center",
              justifyContent: "center",
              borderWidth: 1,
              borderColor: isDark ? "#2a2a2a" : "#E8EAED",
              shadowColor: "#000",
        ...
        >
          <View style={styles.tokenLeft}>
            ...
            <View style={{ marginLeft: 14 }}>
              <Text style={[styles.tokenName, { color: T.text }]}>
...
const MarketTicker = memo(
  ({
    ...
    const tickerCoins = useMemo(() => {
      ...
      return syms.sort(
        (a, b) =>
          Math.abs(prices[b]?.change24h ?? 0) -
          Math.abs(prices[a]?.change24h ?? 0),
      );
    }, [prices]);
    ...
    return (
      <View style={tickerStyles.container}>
        ...
        <View
          style={[
            tickerStyles.surface,
            { backgroundColor: T.surface, borderColor: T.border },
          ]}
        >
          <View
            style={[tickerStyles.accentLine, { backgroundColor: T.primary }]}
          />
            ...
            {isInitialLoad || tickerCoins.length === 0 ? (
              ...
              >
                {items.map((sym, idx) => {
                  const p = prices[sym];
                  const usdVal =
                    typeof p?.usd === "number" && isFinite(p.usd) && p.usd > 0
                      ? p.usd
                      : null;
                  const chg =
                    typeof p?.change24h === "number" && isFinite(p.change24h)
                      ? p.change24h
                      : 0;
                  const isUp = chg >= 0;
                  return (
                    ...
                    >
                      ...
                      <View style={tickerStyles.itemInfo}>
                        <View>
                          <Text
                            style={[tickerStyles.symbolText, { color: T.text }]}
                          >
                            {sym}
                          </Text>
                          <Text
                            style={[
                              tickerStyles.priceText,
                              { color: T.textMuted },
                            ]}
                          >
                            {usdVal !== null
...
const tickerStyles = StyleSheet.create({
  ...
  skeletonCard: {
    ...
    paddingHorizontal: 10,
    gap: 10,
    backgroundColor: "rgba(236,38,41,0.04)",
  },
});
...
const NewsCard = memo(({ item, T }: { item: NewsItem; T: any }) => {
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        <View
          style={[styles.newsSourcePill, { backgroundColor: T.primary + "18" }]}
        >
          <Text style={[styles.newsSourceText, { color: T.primary }]}>
            {item.source?.title}
          </Text>
        </View>
        ...
        <Text style={[styles.newsMeta, { color: T.textMuted }]}>
...
const ChangePill = memo(({ assetsList, T }: { assetsList: any[]; T: any }) => {
  const avgChange = useMemo(() => {
    ...
    const sum = assetsList.reduce((s: number, a: any) => {
      const c =
        typeof a.change24h === "number" && isFinite(a.change24h)
          ? a.change24h
          : 0;
      return s + c;
    }, 0);
  ...
  return (
    ...
    >
      <View
        style={[
          styles.changePill,
          { backgroundColor: isUp ? T.success + "20" : T.error + "20" },
        ]}
      >
        <Feather
...
export default function HomeScreen({ navigation }: any) {
  ...
  const totalUsd = useMemo(() => {
    const sum = assetsList.reduce(
      (acc, a) =>
        acc + (typeof a.usd === "number" && isFinite(a.usd) ? a.usd : 0),
      0,
    );
  ...
  return (
    <View style={[styles.container, { backgroundColor: T.background }]}>
      ...
      <Modal
        ...
        animationType="slide"
        onRequestClose={() => setShowAddrSheet(false)}
      >
        <Pressable
          style={{
            flex: 1,
          ...
          onPress={() => setShowAddrSheet(false)}
        >
          <Pressable
            style={[styles.addrSheet, { backgroundColor: T.surface }]}
          ...
          >
            ...
            >
              <View
                style={[styles.addrChainDot, { backgroundColor: "#627EEA" }]}
              />
              <View style={{ flex: 1 }}>
                <Text style={[styles.addrChainLabel, { color: T.textMuted }]}>
                  Ethereum / EVM
                </Text>
                <Text
                  style={[styles.addrFull, { color: T.text }]}
                  numberOfLines={1}
                >
                  {walletAddress}
                </Text>
              ...
              >
                <Feather
                  name={addrCopied === "ETH" ? "check" : "copy"}
                  size={14}
                  color={addrCopied === "ETH" ? T.success : T.primary}
                />
                <Text
                  style={{
                    fontSize: 11,
            ...
            >
              <View
                style={[styles.addrChainDot, { backgroundColor: "#EF0027" }]}
              />
              <View style={{ flex: 1 }}>
                <Text style={[styles.addrChainLabel, { color: T.textMuted }]}>
                  TRON (TRC20)
              ...
              >
                <Feather
                  name={addrCopied === "TRON" ? "check" : "copy"}
                  size={14}
                  color={addrCopied === "TRON" ? T.success : T.primary}
                />
                <Text
                  style={{
                    fontSize: 11,
      ...
      >
        ...
        >
          <View
            style={[styles.avatarWrap, { backgroundColor: T.primary + "18" }]}
          >
            <Image
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              <Text
                style={[styles.walletLabel, { color: T.text }]}
                numberOfLines={1}
              >
                {walletName}
              </Text>
              ...
              >
                {isSyncing && (
                  <View
                    style={[styles.syncDot, { backgroundColor: networkColor }]}
                  />
                )}
      ...
      <ScrollView
        contentContainerStyle={[
          styles.scroll,
          { paddingTop: headerHeight + 8, paddingBottom: 24 },
        ]}
      ...
      >
        ...
        {priceError && (
          <TouchableOpacity
            style={[
              styles.errorBanner,
              { backgroundColor: T.error + "15", borderColor: T.error + "40" },
            ]}
        ...
        <View style={styles.balanceSection}>
          ...
          >
            <Text style={[styles.estText, { color: T.textMuted }]}>
          ...
          {Object.entries(lockedBalance ?? {}).some(
        ...
        {accountType === "business" ? (
          // Business Merchant: 4 quick actions
          <View style={styles.actionsRow}>
            ...
            <ActionBtn
              icon="repeat"
              label="P2P"
              onPress={() => navigation.navigate("P2PMarketplace")}
              T={T}
              isDark={isDarkMode}
            />
            <ActionBtn
              icon="list-alt"
              label="Orders"
              onPress={() => navigation.navigate("MyP2POrders")}
              T={T}
              isDark={isDarkMode}
            />
            <ActionBtn
        ...
        >
          ...
          >
            ...
            >
              <View
                style={[styles.sectionTitleDot, { backgroundColor: T.primary }]}
              />
          ...
          {isInitialLoad ? (
            ...
            <View style={{ paddingVertical: 24, alignItems: "center", gap: 8 }}>
              ...
              />
              <Text
                style={{ color: T.textMuted, fontSize: 14, fontWeight: "700" }}
              >
                No assets yet
              </Text>
              <Text
              ...
              >
                Deposit or swap to get started
              </Text>
            </View>
          ) : (
